...
| **NIFC WFIGS** (ArcGIS Feature Service) | Confirmed wildfire incident locations (distinct from FIRMS' raw satellite thermal detections) | U.S. public domain | "Wildfire incidents: National Interagency Fire Center (NIFC) — WFIGS" |
...
| **WiGLE API** (`api.wigle.net`) | Viewport-bounded crowdsourced Wi-Fi network observations (optional, BYOK) | [WiGLE EULA](https://wigle.net/eula.html) — free-tier queries are rate-limited per account (sliding scale, resets at US/Pacific midnight); commercial use requires a separate license | "Wi-Fi network observations: WiGLE.net" — registered when a keyed search returns data |
...
| **OpenStreetMap (Overpass API) / DeFlock** | Viewport-bounded public ALPR / Flock Safety camera locations | ODbL 1.0 | "DeFlock contributors via © OpenStreetMap contributors" |
| **Meshtastic public MQTT broker** (mqtt.meshtastic.org) | Opt-in public MapReport node locations | Public broker, published read-only subscriber credentials; each node's data is only what its owner opted to publish | "Meshtastic nodes: opt-in public MapReport data via Meshtastic's public MQTT broker" |
| **Your own TAK Server** (BYOS) | Cursor-on-Target (CoT) team/unit positions and markers you're already authorized to see | Not a God's Eye View data source — it's your own private network, reached only with credentials/certificates you configure | None — not public data; no attribution applies |
...
| **OpenSeaMap** (`tiles.openseamap.org`) | Sea marks raster overlay (buoys, lighthouses, ports) — one of two Open Map Overlays (#10) | Map data ODbL 1.0 via OpenStreetMap | "© OpenSeaMap contributors" |
| **OpenSnowMap** (`opensnowmap.org`) | Ski piste raster overlay — one of two Open Map Overlays (#10) | Map data ODbL 1.0 (OpenStreetMap) + SRTM; map style CC BY-SA | "Map data: © OpenStreetMap contributors, SRTM \| Map style: © OpenSnowMap.org (CC-BY-SA)" |
| **RainViewer** (`api.rainviewer.com`, `tilecache.rainviewer.com`) | Precipitation radar + infrared satellite raster overlay (#85) | [RainViewer API terms](https://www.rainviewer.com/api.html) — free, keyless, attribution required | "© RainViewer" |
...
- **NIFC wildfire incidents.** Fetched directly from NIFC's public, keyless, CORS-open ArcGIS Feature Service (`WFIGS_Incident_Locations_Current`) — no server proxy, same pattern as the earthquakes layer's direct USGS fetch. Scoped to `IncidentTypeCategory IN ('WF','CX')` (wildfires and incident complexes); prescribed burns (`RX`) are excluded at the query level as out of scope for a wildfire-incidents layer. This is confirmed, human-reported incident data — name, cause, acreage, percent contained — distinct from the FIRMS layer's raw satellite thermal-anomaly detections; the two are complementary, not duplicative.
- **Open Map Overlays (#10).** Two independently toggleable keyless raster overlays — OpenSeaMap sea marks and OpenSnowMap ski pistes — drawn as `Cesium.ImageryLayer`s directly on the Cesium globe surface. Like every globe-surface source, they render on the `osm`/`bing-aerial`/`bing-labels` map stacks and are invisible on the default `photoreal` stack (Google Photorealistic 3D Tiles render with the globe hidden; see `mapStackController.js`) — the layer's stats surface that as plain text instead of looking broken. OpenRailwayMap and OpenInfraMap, also requested in #10, are not included: OpenRailwayMap's public tile server enforces a Referer allowlist restricted to its own site (confirmed by direct request — any other Referer, including none, 403s), and OpenInfraMap serves MapLibre vector tiles rather than raster XYZ tiles, which would need a genuine vector-tile rendering pipeline, not an imagery-layer drop-in.
- **Weather radar / satellite overlay (#85).** Two independently toggleable overlays — precipitation radar and infrared satellite cloud cover — fetched directly from RainViewer's public, keyless, CORS-open API (`api.rainviewer.com/public/weather-maps.json` for the current frame list, tiles from `tilecache.rainviewer.com`), no server proxy, same direct-fetch pattern as the NIFC and earthquakes layers. Draws the latest frame only (RainViewer's `radar.past` array carries the last ~2h if a future pass adds animation); satellite defaults off since RainViewer's free satellite feed is intermittently empty. Same globe-surface visibility rule as the Open Map Overlays above — invisible (not broken) on the `photoreal` map stack.
...
- **ALPR / Flock cameras.** Queries the generic `/api/overpass` proxy directly for `man_made=surveillance` + `surveillance:type=ALPR` nodes inside a maximum 3° viewport (tighter than installations — ALPR nodes are far denser, 336K+ worldwide as of 2026), capped at 600 results per query. This is mapped, community-tagged camera *locations and public metadata only* — never plate records, vendor accounts, or private Flock/OSM data. Operator/manufacturer tags containing "Flock" render distinctly from other tagged ALPR vendors; a `camera:direction`/`direction` tag draws a short facing-direction line. Coverage reflects what DeFlock/OSM contributors have mapped, not a complete survey.
- **WiGLE Wi-Fi networks.** Optional and BYOK: without `WIGLE_API_NAME`/`WIGLE_API_TOKEN` set, `/api/wigle/search` reports `available:false` and the layer shows UNAVAILABLE — it never simulates observations. With credentials, requests are viewport-bounded (max 0.4° per side, matching WiGLE's own small and account-dependent daily query allowance), cached 24h in memory and on disk (`.gev-cache/wigle/`, access points don't move fast), and counted against an in-memory daily soft cap (`WIGLE_DAILY_QUERY_BUDGET`, default 50 — tune it down for a new WiGLE account, whose real limit is often lower). Results are crowdsourced *observations*, not live RF detections, and are labeled as such; the "Wi-Fi network observations: WiGLE.net" credit is registered only once a keyed search actually returns data.
- **TAK / Cursor-on-Target (Phase 1, issue #7).** BYOS — God's Eye View never discovers or connects to any TAK network on its own. `/api/tak/events` connects only when `TAK_SERVER_HOST`, `TAK_CLIENT_CERT_PATH`, and `TAK_CLIENT_KEY_PATH` are all explicitly set (an optional `TAK_CA_CERT_PATH` verifies a private/self-signed server; the system trust store is used otherwise — the connection always verifies the server certificate). The server holds one mutual-TLS TCP connection to your TAK Server's streaming input, decodes only CoT `<event>` documents that carry a usable `<point>` (routes/polygons/shapes are out of scope for this pass), and serves the current snapshot as JSON — in memory only, never written to disk, so a restart drops all position history. Each object keeps its original CoT UID, type, callsign, position, timestamp, and stale time, and is dropped from the snapshot the moment its own `stale` time passes. This is strictly read-only ingest: nothing is ever sent onto the TAK stream (no presence announce, no Phase 2 publish-to-TAK).
- **Meshtastic nodes.** `/api/meshtastic/nodes` is backed by one persistent server-side MQTT connection to `mqtt.meshtastic.org` (the project's own public broker, published read-only credentials), subscribed only to the public `msh/+/2/map/#` MapReport topic. It decodes exclusively unencrypted MapReport packets (portnum 73, opt-in "Map Reporting") and never inspects or attempts to decrypt any other Meshtastic MQTT traffic — encrypted channel packets are ignored outright. A node's approximate position is rendered as a translucent uncertainty circle sized from its own reported `position_precision`, never sharpened into an artificially exact pin. An in-memory cache (capped at 8,000 nodes, swept every 15 minutes) ages entries out after 24h of silence; the client shows a node at reduced opacity once it's gone 6h without a report.
