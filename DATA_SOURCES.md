...
- **WiGLE Wi-Fi networks (bring your own account).** `/api/wigle/search` queries [WiGLE](https://wigle.net)'s `network/search` API with HTTP Basic auth using **the operator's own `WIGLE_API_NAME`/`WIGLE_API_TOKEN`**, which never reach the browser. No credentials, no layer: it reports `available:false` and renders UNAVAILABLE rather than simulating data. No WiGLE data is bundled in this repository — it is fetched at runtime and cached only on the machine that ran the query (24h, `.gev-cache/`, which is git-ignored). Viewport is capped at 0.4°, repeats are served from cache, concurrent pans coalesce to one query, and a soft daily counter (`WIGLE_DAILY_QUERY_BUDGET`, default 50, rolling at US/Pacific midnight to match WiGLE's own reset) backstops the account's allowance. Observations are crowdsourced sightings with a `lasttime` that is often years old; the layer buckets and renders that age rather than implying a network is still present.

  **Licensing — read before enabling this anywhere shared.** The [WiGLE data licence](https://wigle.net/eula.html) grants use of the data "on a single computer (i.e., with a single CPU)", requires that the licensee "guarantee that there is no possibility of its use by more than one person at a time", and forbids renting, leasing, sublicensing, transferring, distributing, telecommunicating, copying or selling the data "in its entirety or in any part for any commercial consideration, gain, or benefit". WiGLE additionally requires a separate commercial licence for money-making use, issued with its own tokens. So: each user supplying their own account for their own local instance is what this layer is built for, and **a shared, multi-user, or hosted deployment needs a commercial licence from WiGLE, or must leave this layer disabled.** Same shape as the Google News restriction on the regional briefing above.
