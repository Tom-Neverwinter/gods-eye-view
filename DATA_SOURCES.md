...
- **ALPR / Flock cameras.** Queries the generic `/api/overpass` proxy directly for `man_made=surveillance` + `surveillance:type=ALPR` nodes inside a maximum 3° viewport (tighter than installations — ALPR nodes are far denser), snapped outward to a 0.125° grid so repeated pans reuse one cached Overpass query, capped at 2500 results per query. This is mapped, community-tagged camera *locations and public metadata only* — never plate records, vendor accounts, or private Flock/OSM data. Operator/manufacturer tags containing "Flock" render distinctly from other tagged ALPR vendors; a `camera:direction`/`direction` tag draws a short facing-direction line. Coverage reflects what DeFlock/OSM contributors have mapped, not a complete survey.
