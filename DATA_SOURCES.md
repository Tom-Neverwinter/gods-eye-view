...
- **Rayhunter cell-site-simulator warnings (opt-in, your own device).** `/api/rayhunter/*` relays two read-only endpoints from a [Rayhunter](https://github.com/EFForg/rayhunter) device the user owns and runs themselves — typically a flashed Orbic mobile hotspot, reached over USB tether or its own Wi-Fi. The device address is supplied by the user at runtime and is constrained to loopback, an RFC1918 private network, or a `.local` name, so the proxy cannot be pointed at a public host. No data leaves the user's machine and nothing is written to the device; the layer is off by default. **Warnings are plotted at wherever the browser is when the warning is first observed, which is NOT the device's location at detection time** — this app has no way to know that. The layer states that approximation in its own status rather than presenting the position as precise. Rayhunter's analyzers report suspicious cellular behaviour, not confirmed surveillance. AGPL-3.0; © EFF and Rayhunter contributors.
