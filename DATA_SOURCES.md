...
- **Weather radar / satellite overlay (#85).** Two independently toggleable overlays — precipitation radar and infrared satellite cloud cover — fetched directly from RainViewer's public, keyless, CORS-open API (`api.rainviewer.com/public/weather-maps.json` for the current frame list, tiles from `tilecache.rainviewer.com`), no server proxy, same direct-fetch pattern as the earthquakes layer's direct USGS fetch. Draws the latest frame only (RainViewer's `radar.past` array carries the last ~2h if a future pass adds animation); satellite defaults off since RainViewer's free satellite feed is intermittently empty. Drawn as a `Cesium.ImageryLayer` directly on the Cesium globe surface: visible on the `osm`/`bing-aerial`/`bing-labels` map stacks, invisible (not broken) on the default `photoreal` stack (Google Photorealistic 3D Tiles render with the globe hidden; see `mapStackController.js`) — the layer's stats surface that as plain text instead of looking broken.
